...
from .forms import ClassForm, SubjectForm, TeachingForm
...
def CourseListView(request, id):
    courses = Subject.objects.filter(Class=id)
    context = {
        'courses': courses
    }
...
class ConfirmDeleteCourseView(View, LoginRequiredMixin):
    def get(self, request, courses_id, course_id):
        courses = Subject.objects.get(id=courses_id)
        course = courses.course_set.get(id=course_id)
        return render(request, 'courses/confirm_delete.html', {"courses": courses, "course": course})
...
class DeleteCourseView(View, LoginRequiredMixin):
    def get(self, request, courses_id, course_id):
        ...
        return redirect(reverse("courses:course_detail", kwargs={"id": courses.id}))
...
def create_legend(request):
    ...
    return render(request, 'courses/create_stuff.html', context)
